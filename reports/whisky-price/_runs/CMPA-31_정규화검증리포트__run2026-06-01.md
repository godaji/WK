...
- 입력 raw 행: **4,945**  ·  distinct raw 표기: **4,549**
- 정규화(병합) 후 정본 SKU 수: **88** (whisky-list.csv 89종 중)
- 통합 데이터셋(clean 단품) 행: **713**
- 오염행 제거(과거평균 ⅓~3배 밖): **46**

> ⚠️ 신뢰도: 국내(한글) 매칭은 명시적 사전(match/not) 기반 고신뢰. 해외 매칭(78행, reason=`en`)은 name_en 브랜드+년수 토큰 휴리스틱이라 보조(advisory) 신뢰도 — 리포트 본표 반영 전 스팟체크 권장. 모호(후보 2개↑)는 미매칭 처리.

## 소스별 정규화/제외 집계

| 소스 | 입력스킵 | matched | unmatched | 비위스키제외 | 비단품제외 | 가격결측 | 오염제거 |
|---|--:|--:|--:|--:|--:|--:|--:|
| youtube_martweb | 0 | 567 | 105 | 17 | 25 | 57 | 12 |
| dailyshot | 130 | 83 | 12 | 0 | 0 | 0 | 3 |
| overseas | 0 | 109 | 3,947 | 0 | 23 | 0 | 31 |
| **합계** | 130 | 759 | 4,064 | 17 | 48 | 57 | 46 |
...
## 미매칭 raw 표기 (4066종) — 마스터 SKU 확장 후보
...
- … 외 4026종 (전체는 `assets/whisky-aliases.csv` status=unmatched)
