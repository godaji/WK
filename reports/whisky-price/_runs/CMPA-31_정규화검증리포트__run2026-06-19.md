...
- 입력 raw 행: **5,227**  ·  distinct raw 표기: **4,632**
- 정규화(병합) 후 정본 SKU 수: **139** (whisky-list.csv 89종 중)
- 통합 데이터셋(clean 단품) 행: **1,133**
- 오염행 제거(과거평균 ⅓~3배 밖): **76**

> ⚠️ 신뢰도: 국내(한글) 매칭은 명시적 사전(match/not) 기반 고신뢰. 해외 매칭(115행, reason=`en`)은 name_en 브랜드+년수 토큰 휴리스틱이라 보조(advisory) 신뢰도 — 리포트 본표 반영 전 스팟체크 권장. 모호(후보 2개↑)는 미매칭 처리.

## 소스별 정규화/제외 집계

| 소스 | 입력스킵 | matched | unmatched | 비위스키제외 | 비단품제외 | 가격결측 | 오염제거 |
|---|--:|--:|--:|--:|--:|--:|--:|
| youtube_martweb | 0 | 677 | 40 | 18 | 7 | 0 | 5 |
| dailyshot | 117 | 89 | 1 | 0 | 0 | 0 | 2 |
| overseas | 0 | 183 | 3,891 | 0 | 25 | 0 | 68 |
| **합계** | 117 | 949 | 3,932 | 18 | 32 | 0 | 75 |
...
## 미매칭 raw 표기 (3958종) — 마스터 SKU 확장 후보
...
- … 외 3918종 (전체는 `assets/whisky-aliases.csv` status=unmatched)
